...
export const getMockData = (pageNum) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const start = PER_PAGE * (pageNum - 1);
      const end = PER_PAGE * pageNum;
      const datas = MOCK_DATA.slice(start, end);
      const isEnd = end >= MOCK_DATA.length;
...
const InfiniteScroll = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const newTotalSum = datas.reduce((acc, item) => acc + item.price, 0);
      setTotalSum((prevTotal) => prevTotal + newTotalSum);
    } catch (error) {
      ...
      <ul className="product-list">
        {dataList.map((item) => (
          <li key={item.productId} className="product-card">
            ...
            <p>구매일자: {new Date(item.boughtDate).toLocaleDateString()}</p>
          </li>
        ))}
      </ul>
      <h3 className="total-sum">총 가격: {totalSum.toLocaleString()}원</h3>
      {isLoading && <div className="loader">로딩 중...</div>}
      {!isEnd && <div ref={loaderRef} className="observer"></div>}
      {isEnd && <p className="end-message">모든 데이터를 불러왔습니다.</p>}
    </div>
  );
};
